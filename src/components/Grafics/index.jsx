...
export const Grafics = () => {
  ...
  const expenses = data.filter(
    (item) =>
      (inputMonth !== "Todos"
        ? parseInt(new Date(item.data).getMonth() + 1) === parseInt(inputMonth)
        : true) && item.tipo === "Gasto"
  );
  const incomes = data.filter(
    (item) =>
      (inputMonth !== "Todos"
        ? parseInt(new Date(item.data).getMonth() + 1) === parseInt(inputMonth)
        : true) && item.tipo === "Ganho"
  );
  const expAndInc = data.filter((item) =>
    inputMonth !== "Todos"
      ? parseInt(new Date(item.data).getMonth() + 1) === parseInt(inputMonth)
      : true
  );
  ...
  const filterCatExpenses = catExpenses.filter(
    (cat, i) => catExpenses.indexOf(cat) === i
  );
  const filterCatIncomes = catIncomes.filter(
    (cat, i) => catIncomes.indexOf(cat) === i
  );
  ...
  const valueExpensesCat = filterCatExpenses.map((item) =>
    expenses
      .map((el) => (el.categoria === item ? parseFloat(el.valor) : 0))
      .reduce((c, t) => c + t, 0)
  );
  ...
  const valueTipes = filterTipes.map((item) =>
    expAndInc
      .map((el) => (el.tipo === item ? parseFloat(el.valor) : 0))
      .reduce((c, t) => c + t, 0)
  );
  ...
  const valueExpensesLine = filterMonths.map((item) =>
    valueExpenses
      .map((el) =>
        parseInt(new Date(el.data).getMonth() + 1) === parseInt(item)
          ? parseFloat(el.valor)
          : 0
      )
      .reduce((c, t) => c + t, 0)
      ...
      <section className="container-Grafics">
        ...
        <div className="filtros">
          ...
          >
            ...
            <option value="Evolução financeira">
              Gráfico de evolução financeira
            </option>
            <option value="Gastos por categoria">
              Gráfico de gastos por categoria
            </option>
            <option value="Ganhos por categoria">
