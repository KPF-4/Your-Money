...
const Quotation = () => {
  ...
  useEffect(() => {
    setTotal(
      parseFloat(
        parseFloat(inputValue) * parseFloat(quotation[nameQuotation]?.ask)
      ).toFixed(2)
    );
      ...
      <StyleQuotation>
        ...
        {names.map((el, index) => (
          <div key={index} className="list">
            <StyleSpan type="coin">
              {listQuotation[el].code + "/" + listQuotation[el].codein}
            </StyleSpan>
            <StyleSpan type="bid" color="#008000">
              {listQuotation[el].bid}
            </StyleSpan>
            <StyleSpan type="ask" color="#FF0000">
              {listQuotation[el].ask}
            </StyleSpan>
            <StyleSpan
              type="var"
              color={listQuotation[el].pctChange > 0 ? "#008000" : "#FF0000"}
            >
      ...
      {display === true ? (
        ...
        <StyleForm onSubmit={handleSubmit(onSubmitConvert)}>
          ...
          <div>
            <input
              placeholder={"Valor"}
              {...register("value", { required: "Digite o valor desejado!" })}
            />
